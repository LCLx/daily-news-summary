...
Two parallel pipelines, both fetching from the same RSS sources (`RSS_SOURCES` in `src/config.py`):
...
The email pipeline is split into focused modules under `src/`:

| Module | Responsibility |
|---|---|
| `config.py` | `RSS_SOURCES`, env vars, `CATEGORY_EMOJIS`, `CLAUDE_MAX_RETRIES` |
| `rss.py` | `extract_image_url()`, `fetch_rss_articles()` |
| `claude_client.py` | `generate_summary_with_claude()` — prompt loading + API tool calling / CLI call with json_repair fallback |
| `digest.py` | `resolve_references()` — maps Claude's JSON refs to full article data |
| `renderer.py` | `build_email_html_from_json()` — renders sections to HTML |
| `mailer.py` | `send_email_gmail()` — Gmail SMTP delivery |
| `email_pipeline.py` | `main()` — orchestrates the pipeline |

Static assets:
- `src/prompts/email_digest.md` — Claude prompt template (`$articles` placeholder for dynamic content)
- `src/templates/email.html` — HTML email wrapper/CSS (`$date_str`, `$body_html` placeholders)

Both pipelines share `config.py` and `rss.py`.
...
RSS sources are defined in `RSS_SOURCES` in `config.py`, grouped by 6 categories: Tech & AI, Global Affairs, Business & Finance, Pacific Northwest, Health & Science, Deals.
